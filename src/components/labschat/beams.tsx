...
/**
 * Adaptado do componente "Beams" (React Bits, https://reactbits.dev) para
 * TypeScript. Usado como background ambiente da landing /labschat — não é
 * um componente de propósito geral do design system do site.
 */
...
function extendMaterial(
  ...
  // biome-ignore lint/suspicious/noExplicitAny: three's internal ShaderLib typings don't cover "physical".
  ...
  // biome-ignore lint/suspicious/noExplicitAny: generic material constructor, properties vary by subclass.
...
const DirLight = ({ position, color }: { position: [number, number, number]; color: string }) => {
  ...
  useEffect(() => {
    ...
    cam.right = 24;
    cam.far = 64;
    if (dir.current) dir.current.shadow.bias = -0.004;
  }, []);
